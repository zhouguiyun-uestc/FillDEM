...
void setNoData(BYTE* data, int length, BYTE noDataValue);
void setNoData(float* data, int length, float noDataValue);
void setFlag(int index, BYTE* flagArray);
bool isProcessed(int index, const BYTE* flagArray);
...
public:
	...
	bool Init(int width,int height)
	{
		...
		this->width=width;
		this->height=height;
		flagArray = new BYTE[(width * height + 7) / 8]();
